...
const CustomTimePicker = ({
  ...
  const selectedMinute = Math.min(59, Math.max(0, parseInt(minuteText, 10) || 0));
  ...
  const stepHour = (delta: number) => {
    const { hour } = fromMinutes(toMinutes(selectedHour + delta, selectedMinute));
    applyTime(hour, selectedMinute);
  };

  const stepMinute = (delta: number) => {
    const { hour, minute } = fromMinutes(toMinutes(selectedHour, selectedMinute) + delta);
  ...
  const applyRelativeOffset = (deltaMinutes: number) => {
    ...
    const { hour, minute } = fromMinutes(toMinutes(base.hour, base.minute) + deltaMinutes);
    applyTime(hour, minute);
  };
        ...
        >
          ...
          <View style={styles.timeInputSection}>
            <View style={styles.timeInputGroup}>
              <TouchableOpacity style={styles.stepperButton} onPress={() => stepHour(1)}>
            ...
            <View style={styles.timeInputGroup}>
              <TouchableOpacity style={styles.stepperButton} onPress={() => stepMinute(5)}>
                <Ionicons name="chevron-up" size={20} color="#2575fc" />
              </TouchableOpacity>
              <TextInput
                ref={minuteInputRef}
                ...
                <View style={styles.presetButtons}>
                  {group.presets.map((preset) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={preset.label}
                        style={[styles.presetButton, active && styles.activePreset]}
                        onPress={() => applyTime(preset.hour, preset.minute)}
                      >
                        <Text style={[styles.presetButtonText, active && styles.activePresetText]}>
                          {preset.label}
                        </Text>
                      </TouchableOpacity>
                    );
